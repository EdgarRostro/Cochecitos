...
def cars():
    ...
    if request.method == 'GET':
        ...
        carsPositions = [{"x": agent.pos[0], "y": agent.pos[1], "directionLight": agent.directionLight, "isParked": agent.is_parked} for agent in city.schedule.agents if isinstance(agent, Car)]
...
def trafficLights():
    ...
    if request.method == 'GET':
        if city == None:
            return jsonify({"error": "Model is not initialized. Initialize and try again"})
        trafficLightsStates = [{"state": agent.state} for agent in city.schedule.agents if isinstance(agent, Traffic_Light)]
